...
import { useSubirValeBlob } from '@/features/almacen/api/useSalidas';
...
export interface ValeUploadValue {
  blobRef: string;
...
export interface ValeUploadProps {
  value: ValeUploadValue | null;
  onChange: (value: ValeUploadValue | null) => void;
  disabled?: boolean;
  error?: string;
}

/**
 * <c>&lt;ValeUpload/&gt;</c> — drag-and-drop + file picker para el vale
 * escaneado/firmado de Variante B (FE-F3-PR1, A14). Mismas reglas que
 * PackingListUpload pero contra el endpoint
 * <c>/api/v1/almacen/salidas/vale/blob</c> (PR #258).
 */
export function ValeUpload({
  value,
  onChange,
  disabled,
  error,
  ...
  async function procesarArchivo(file: File) {
    ...
    try {
      ...
      onChange(result);
      toast.success(`Vale subido: ${result.nombreArchivo}`);
    } catch (err) {
      if (esApiError(err)) {
      ...
      >
        {subir.isPending ? (
          <div className="flex flex-col items-center gap-2 text-muted-foreground">
            ...
            <p className="text-sm">Subiendo vale…</p>
          </div>
        ) : (
          <>
            <Upload className="mx-auto h-6 w-6 text-muted-foreground" />
            <p className="mt-2 text-sm text-muted-foreground">
              Arrastra el vale firmado aquí, o
              <button
                type="button"
