...
Si se dispone de un tenant de desarrollo autorizado, también puede probarse **Graph para crear/vincular la cuenta y Mailpit para capturar el correo**, sin buzón Exchange ni `SenderEmail`: configurar explícitamente `Entra:Proveedor=Graph`, `Entra:Correo:Proveedor=SandboxLocal`, `Entra:Provision:Disabled=false`, credenciales Graph de prueba, dominios permitidos, `Entra:UrlInicioSesion` HTTPS y SMTP de captura en loopback. Este modo solo arranca en `Development`. El correo distingue que la cuenta y la clave de Microsoft son reales, pero que el mensaje **no se entregó al destinatario**. Para probar login real debe configurarse además `Auth:Mode=EntraId`; esto no queda probado por el flujo simulado anterior. En cualquier otro ambiente, `Graph` sigue exigiendo `SenderEmail` para usar correo Graph y rechaza `SandboxLocal`.
...
Para dejar ADM-01/02 **listas para QA de desarrollo**, basta con build y pruebas locales en verde, vínculo empleado–usuario persistido, alta y gestión de acceso sin romper permisos ni roles/sucursales, y recorrido de correo capturado/simulado registrado por QA. **No se exige Exchange ni entrega real del correo para este corte.** La aceptación final por Millet y la prueba de entrega real en su tenant son hitos posteriores y separados; no se infieren de esta prueba.
